...
<div class="content">
 ...
 <?php
   ...
   // echo $sname1;
    ...
    $query= "SELECT initcap(st.fname),initcap(st.lname),username FROM st WHERE (lower(st.fname) LIKE '%".$sname1."%' or lower(st.lname) LIKE '%".$sname1."%') AND username != '".$username."'";
    $st=  oci_parse($conn, $query);
    oci_execute($st);
    ...
    echo 
        "  
            <tr>
            <td class=\"w3-large\"><b><div class=\"w3-card w3-green w3-container\">Name:</b> ".$fullname." </div><br />
            <td><form><button class=\"w3-btn w3-border-yellow w3-round-large w3-ripple w3-blue\" type=\"submit\" formaction=\"search_profile.php\" formmethod=\"get\" value=\"".$uname."\" name=\"vprofile_btn\">View Profile</button><br /></form><br/> </td>
            </tr>
    ...
    $query1= "SELECT INITCAP(fname),INITCAP(lname),username FROM Inst WHERE (lower(fname) LIKE '%".$sname1."%' or lower(lname) LIKE '%".$sname1."%') and username != '".$username."'";
    $st1=  oci_parse($conn, $query1);
    oci_execute($st1);
    while(($row = oci_fetch_array($st1))!=False) {
    // var_dump($row);
    // var_dump($row);
    $fullname=$row[0]." ".$row[1];
    $uname=$row[2];
    ...
    $query1= "SELECT INITCAP(fname),INITCAP(lname),username FROM rec WHERE (lower(fname) LIKE '%".$sname1."%' or lower(lname) LIKE '%".$sname1."%') and username != '".$username."'";
    $st1=  oci_parse($conn, $query1);
    oci_execute($st1);
    while(($row = oci_fetch_array($st1))!=False) {
    // var_dump($row);
