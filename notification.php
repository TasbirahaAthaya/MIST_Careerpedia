  ...
  <div class="container">
      ...
      <?php
        ...
        if($noti_num == 0)
        {
            echo "
                <script>
                    alert(\"No new offer alerts\");
                    location.replace(\"Student-profile.php\");
                </script>
            ";            
        }
        else{
            $query = "SELECT * FROM user_notification WHERE Username_rcvr  = '".$username."'";
            $st=  oci_parse($conn, $query);
            oci_execute($st); 
            $ctr=1;
            while(($row = oci_fetch_array($st)) != false)
            {
                ...
                $query2="SELECT * FROM ".$stb." WHERE C_id='".$row[2]."'";
                $st2=  oci_parse($conn, $query2);
                oci_execute($st2);
                $rrow=  oci_fetch_array($st2);
                if($rrow[0] == "")
                {goto end_this;}
                echo"
                    <div class = \"w3-panel\">
                    <b>".$ctr++."</b>
                    <p>Offer ID: ".$rrow[0]."</p>
                    <p>Required Department: ".$rrow[1]."
                    <p>Required Degree: ".$rrow[2]."
                    <p>Required GPA: ".$rrow[3]."
                    <p>Required Thesis topic: ".$rrow[4]."
                    <p>Required Number of publications: ".$rrow[5]."
                    <p>Offered BY: ".$row[1]."
                    <p>You seem to be qualified for this offer. Do you accept this invitation? </p>
                    <p> <form><button class=\"w3-btn w3-border-yellow w3-round-large w3-ripple w3-blue\" type=\"submit\" formaction=\"
                        ";
                            $nqu="Update ta_circ set avail=0 where C_id='".$rrow[0]."'";
                            $nst=oci_parse($conn,$nqu);
                            $bool2=oci_execute($nst);
                            if($bool2==TRUE)
                            {
                                $nqu="Delete from user_notification where offer_id = '".$rrow[0]."'";
                                $nst=oci_parse($conn,$nqu);
                                $bool2=oci_execute($nst);
                                $nqu="INSERT INTO user_notification VALUES('".$row[1]."','".$username."',0,4)";
                                $nst=oci_parse($conn,$nqu);
                                $bool2=oci_execute($nst);
                            }
                        echo "\" formmethod=\"get\" value=\"".$username."\" name=\"ac_btn\">I ACCEPT</button><br /></form> 
                ";
            ...
            teachers_corn:
                echo "
                    <div class = \"w3-panel\">
                    <b>".$ctr++."</b>
                    <p><b> ".$row[1]." </b>Accepted your invitation</p><p>Please proceed to contact him through his contact details</p>";
